...
?>
<?php
/*Function to load my ajax javascript file.
*Added by Vinay, 04/26/2012
*/
function add_myjavascript(){
 ...
 /*Function to handle the Ajax call made.
*Added by Vinay, 04/26/2012
*/
wp_enqueue_script('jquery');
...
function addNewUser(){
	...
	$name = $_POST['name'];
	$email = $_POST['email'];
	$mypasswd = $_POST['password1'];
	$mypasswd_confirm = $_POST['password2'];

	if($mypasswd != $mypasswd_confirm){
	...
	echo "New user succesfully created. Please log in with your credentials to start using Share Articles.";
	//Send confirmation email to the user that his account has been created.
	$mail = new PHPMailer(true); //defaults to using php "mail()"; the true param means it will throw exceptions on errors, which we need to catch

	try {
	  $mail->AddAddress($email);
	  $mail->SetFrom('do-not-reply@vinpad.us', 'Share Articles');
	  $mail->AddReplyTo('name@yourdomain.com', 'First Last');
	  $mail->Subject = 'Share Articles New User';
	  $mail->AltBody = 'To view the message, please use an HTML compatible email viewer!'; // optional - MsgHTML will create an alternate automatically
	  $mail->MsgHTML('<p>Your new user account has been created at Share Articles.</p><br><p>Username: '.$name.'</p><br><p>Log in at http://vinpad.us/wp/wp-login.php to start using Share Articles.</p>');
	  $mail->Send();
...
function addNewGroup(){
	...
	//$grp_name = $_POST['grp_name'];/*This is commented out now since group name can be non unique. Since we know that group_email should be unique let us use that internally as the 'name' of the category and externally let us have another field called grp_title as the 'NAME' of the group. Internally WP does not allow us to have 2 categories with the same name anyways. So let us internally keep the category/group names unique for our internal use only.*/
	$grp_desc = $_POST['grp_desc'];
	$grp_name= $_POST['grp_email']; /*We will use grp email as both the email alias for the group as well as an internal unique name for the group.*/
	$grp_email= $_POST['grp_email'];
	$grp_invite = $_POST['grp_invite'];

	$grp_invite_list = explode(",", $grp_invite);

	//Add the group

	global $userAccessManager;
	$uamAccessHandler = $userAccessManager->getAccessHandler();
	$userGroups = $uamAccessHandler->getUserGroups(null, true);
	$uamUserGroup 
            = new UamUserGroup($userAccessManager->getAccessHandler(), null);
	$uamUserGroup->setGroupName($grp_name);
	$uamUserGroup->setGroupDesc($grp_desc);
	$uamUserGroup->save();
	$userAccessManager->getAccessHandler()->addUserGroup($uamUserGroup);


	/*New group has been created. Now create a new categroy with the same name as the group
	* and set up access control between the new group and the new category.
	* This has to be done because whenever the user posts, he has to chose the category
	* to which he wants to post. And we are trying to enforce privacy to that category.
	* We do that by tying that category to User Access group that the user newly created.
	*/
	

	
	global $wpdb;
	//Get the ID of the newly created group and category

	$groupID = $uamUserGroup->getId();
	

	//Set the group ID to access_group association
	$categoryID = _setup_group_categroy_association($groupID, $grp_name, $grp_desc, $grp_email);
	


	//Now grant the user that created this group with access privilages to this group.
	//Get the ID of the current user.
	global $current_user;
    get_currentuserinfo();
	//Set the group access of the newly created group for the current user.
	_set_user_group_access($wpdb, $current_user->ID, $groupID);

	echo "New group succesfully created.";

	
	
	$mail = new PHPMailer(true);
		try {
		  $mail->AddAddress($grp_email);
		  $mail->SetFrom('do-not-reply@vinpad.us', 'Share Articles');
		  $mail->AddReplyTo('name@yourdomain.com', 'First Last');
		  $mail->Subject = 'Share Articles: Invited to Group';
		  $mail->AltBody = 'To view the message, please use an HTML compatible email viewer!'; // optional - MsgHTML will create an alternate automatically
		  $mail->MsgHTML('<p>Your have been added to the group '.$grp_name.' by'.$current_user->user_firstname.' '.$current_user->user_lastname.'</p><br><p>Goto http://vinpad.us/wp/?page_id=100 to confirm that you want to join the group</p>');
		  $mail->Send();
...
function setMemberRequestForGroup($uname, $grpname){

	_check_if_user_exists($uname);
	_check_if_group_exists($grpname);



}

function _setup_group_categroy_association($groupID, $grp_name, $grp_desc, $grp_email){
	//Create a new category have the same name as the group.
	$my_cat = array('cat_name' => $grp_name, 'category_description' => $grp_desc, 'category_nicename' => $grp_name, 'category_parent' => '');

	$categoryID= wp_insert_category($my_cat);

	if($categoryID == 0)
	{
		echo "Error while creating the group. Please try again.";
		die();
	}
	
	global $wpdb;
	$group_cat_sql = $wpdb->prepare("INSERT INTO `wp_uam_accessgroup_to_object` (group_id, object_id, object_type) 
	VALUES ( %d, %d, %s )", $groupID, $categoryID, 'category' );
	
	$wpdb->query($group_cat_sql);

	return $categoryID;
}


function _set_user_group_access($wpdb, $userID, $groupID){
	//Function to grant membership of a group to a particular user.
	$group_cat_sql = $wpdb->prepare("INSERT INTO `wp_uam_accessgroup_to_object` (group_id, object_id, object_type) 
	VALUES ( %d, %d, %s )", $groupID, $userID, 'user' );
...
function postToGroups(){
	...
	$category_name = $_POST['selectGroups'];
	$user_name = $_POST['username'];
	$current_url = $_POST['current_url'];
	$_group = array();
	$user_publish_error = false;
	$user_publish_error_msgs = array();
	//Extract all the groups that the user wants to publish to.
	foreach($category_name as $cat){
		//Check if the user is a member of the group.
		$user_member_of_group = _check_user_group_member($user_name, $cat);
		if($user_member_of_group){
			//Allow user to publish only if he is a member of the group.
			array_push($_group, get_cat_ID($cat));
		}
		else{
			//This would be a rare scenario since initially we only present the groups that the user
			//is a member of.
			$user_publish_error = true;
			array_push($user_publish_error_msgs, $cat);
		}
		
	}
	
	//Check if the user is logged in and then publish to all the groups that the user is a member of.
	if (is_user_logged_in()){
		
				$my_post = array(
							 'post_title' => 'Title',
							 'post_content' => '<a href='.$current_url.'>'.$current_url.'</a>',
							 'post_status' => 'publish',
							 'post_author' => $current_user->ID,
							 'post_category' => $_group
						  );
	...
	//Check if there were any publish related errors and display the errors to the user.
	if($user_publish_error){
		echo "Sorry! You donot have permissions to publish to the following groups: ";
		foreach($user_publish_error_msgs as $err){
			echo $err.", ";
		}
	}
	die();
...
function _check_user_group_member($user_name, $category_name){
	//Returns true if the user is a member of the group. Returns False otherwise.
	return true;
}


//Function to hide certain menu items from the user.
add_action( 'admin_menu', 'my_remove_menu_pages' );

function my_remove_menu_pages() {
	remove_menu_page('post_to_group.php');
	
}


add_action('wp_ajax_addNewUser', 'addNewUser');
add_action('wp_ajax_nopriv_addNewUser', 'addNewUser'); // not really needed

add_action('wp_ajax_addNewGroup', 'addNewGroup');
add_action('wp_ajax_nopriv_addNewGroup', 'addNewGroup'); // not really needed

add_action('wp_ajax_makeMemberofGroup', 'makeMemberofGroup');
add_action('wp_ajax_nopriv_makeMemberofGroup', 'makeMemberofGroup'); // not really needed

add_action('wp_ajax_postToGroups', 'postToGroups');
add_action('wp_ajax_nopriv_postToGroups', 'postToGroups'); // not really needed
